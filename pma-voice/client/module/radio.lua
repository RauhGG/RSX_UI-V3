...
function setRadioChannel(channel)
	if channel > 0 then
		TriggerEvent('rsx_ui:setradiochan', channel, true)
	end
	if channel == 0 then
		TriggerEvent('rsx_ui:setradiochan', 0, false)
	end
	type_check({channel, "number"})
	TriggerServerEvent('pma-voice:setPlayerRadio', channel)
	radioChannel = channel
	print('[radio] Setting radio channel to ' .. channel)
end
...
RegisterCommand('testradio', function(source)
	setRadioChannel(3)
end)

RegisterCommand('exitradio', function(source)
	setRadioChannel(0)
end)
...
RegisterCommand('+radiotalk', function()
	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			radioPressed = true
			playMicClicks(true)
			TriggerEvent('rsx_ui:talkingradio', true)
...
RegisterCommand('-radiotalk', function()
	if radioChannel > 0 or radioEnabled and radioPressed then
		...
		TriggerEvent('rsx_ui:talkingradio', false)
		playMicClicks(false)
